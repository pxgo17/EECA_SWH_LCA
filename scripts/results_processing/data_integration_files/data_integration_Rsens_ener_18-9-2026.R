...
raw_technologies <- read_csv("D:/EECA_SWH_LCA/EECA_SWH_LCA/EECA_SWH_LCA/data/processed/thermal/technologies_env_shifted_ocgt.csv")
...
raw_technologies_processed_CS_DW <- raw_technologies_processed %>%
  ...
  left_join(embodied_DW %>% select(DW_code,`Tank_Volume (L)`,Manufacture,Distribution,Installation,EOL,Other,
                                   EOL_S,Other_S, Manufacture_MJ,Distribution_MJ,Installation_MJ,EOL_MJ,Other_MJ), 
...
raw_technologies_processed_CS_DW_SH <- raw_technologies_processed_CS_DW %>%
  ...
  mutate(HP_cap = coalesce(HP_cap_right, HP_cap_left),
         ...
         Manufacture_MJ = coalesce(Manufacture_MJ_right, Manufacture_MJ_left),
  ...
  select(-HP_cap_right, -HP_cap_left, -Manufacture_right,-Manufacture_left,
         ...
         -Other_S_left,
         -Manufacture_MJ_right,-Manufacture_MJ_left,
         -Distribution_MJ_right, -Distribution_MJ_left, -Installation_MJ_right,
         -Installation_MJ_left, -EOL_MJ_right, -EOL_MJ_left, -Other_MJ_right,
         -Other_MJ_left) %>% 
...
raw_technologies_processed_CS_DW_SH_HYD <- raw_technologies_processed_CS_DW_SH %>%
  left_join(embodied_SH_HYD %>% select(house_type,insulation,Loc_code,SH_code,HP_cap,
                                       ...
                                       Hyd_Manufacture_MJ_HP,
                                       Hyd_Manufacture_MJ_Tank, Hyd_Manufacture_MJ_FC),
...
technology_year <- raw_technologies_processed_CS_DW_SH_HYD %>% filter(Grid_year>2024) %>%
  ...
  mutate(MJ_manuf_year = approx(year, Manufacture_MJ/21, xout = year, rule = 2)$y) %>%
...
technology_year %>% write_csv(paste0("D:/EECA_SWH_LCA/EECA_SWH_LCA/EECA_SWH_LCA/data/processed/lca/technology_year_env_shifted_ocgt_",CS,".csv"))
...
technology_lifetime <- technology_year %>% 
  ...
  summarise(kgCO2_oper_life = sum(kgCO2_oper_year),
            ...
            MJ_manuf_life = sum(MJ_manuf_year),
            MJ_dist_life = sum(MJ_dist_year),
            MJ_inst_life = sum(MJ_inst_year),
            MJ_eol_life = sum(MJ_eol_year),
            MJ_other_life = sum(MJ_other_year)
            ) %>% ungroup() %>%
  ...
  mutate(MJ_total_life = (P_annual_kWh_life/0.278) + MJ_manuf_life + # notice conversion from kWh to MJ
           MJ_dist_life + MJ_inst_life + MJ_eol_life + MJ_other_life)
  

# Please make sure you change to desired directory (latest version was saved in github repo)
technology_lifetime %>% write_csv(paste0("D:/EECA_SWH_LCA/EECA_SWH_LCA/EECA_SWH_LCA/data/processed/lca/technology_lifetime_env_shifted_ocgt_",CS,".csv"))
...
if (CS=="C0"){
  ...
  raw_technologies_processed_C0_DW_SH_HYD <- raw_technologies_processed_C0_DW_SH_HYD %>%
    mutate(Manufacture = Manufacture/21,
           ...
           EOL_S= EOL_S/21,
           Other_S = Other_S/21,
           Manufacture_MJ = Manufacture_MJ/21,
           Distribution_MJ = Distribution_MJ/21,
           Installation_MJ = Installation_MJ/21,
           EOL_MJ= EOL_MJ/21,
           Other_MJ = Other_MJ/21
           )
}
...
raw_technologies_processed_SH_R <- raw_technologies_processed_C0_DW_SH_HYD %>%
  ...
  rename(
    ...
    EOL_S_SH = EOL_S,
    Other_S_SH = Other_S,
    Manufacture_SH_MJ = Manufacture_MJ,
    Distribution_SH_MJ = Distribution_MJ,
    Installation_SH_MJ = Installation_MJ,
    EOL_SH_MJ = EOL_MJ,
    Other_SH_MJ = Other_MJ
    )
...
raw_technologies_processed_SH_HP_R <- raw_technologies_processed_C0_DW_SH_HYD %>%
  ...
  # and one for R_cap, and one will be NA.  
            ...
            Manufacture_SH_MJ = sum(Manufacture_MJ),
            Distribution_SH_MJ = sum(Distribution_MJ),
            Installation_SH_MJ = sum(Installation_MJ),
            EOL_SH_MJ = sum(EOL_MJ),
            Other_SH_MJ = sum(Other_MJ)) %>% 
  ungroup() %>%
...
raw_technologies_processed_WH_HP <- raw_technologies_processed_CS_DW_SH_HYD %>%
  ...
  summarise(P_annual_kWh_WH = sum(`P_annual (kWh)`),
            ...
            Manufacture_WH_MJ = sum(Manufacture_MJ),
            Distribution_WH_MJ = sum(Distribution_MJ),
            Installation_WH_MJ = sum(Installation_MJ),
            EOL_WH_MJ = sum(EOL_MJ),
            Other_WH_MJ = sum(Other_MJ)) %>% 
  ungroup()
...
household_SH_WH <- bind_rows(non_hydronic_SH_WH_dataframes) %>%
  mutate(P_annual_kWh_total = P_annual_kWh_SH + P_annual_kWh_WH,
         ...
         Manufacture_MJ = Manufacture_SH_MJ + Manufacture_WH_MJ,
         Distribution_MJ = Distribution_SH_MJ + Distribution_WH_MJ,
         Installation_MJ = Installation_SH_MJ + Installation_WH_MJ,
         EOL_MJ = EOL_SH_MJ + EOL_WH_MJ,
         Other_MJ = Other_SH_MJ + Other_WH_MJ)
...
# Please make sure you change to desired directory (latest version was saved in github repo)
household_interp %>% write_csv(paste0("D:/EECA_SWH_LCA/EECA_SWH_LCA/EECA_SWH_LCA/data/processed/lca/household_year_env_shifted_ocgt_",CS,".csv"))
...
household_embodied <- household %>%
  select(house_type,insulation,
         ...
         Manufacture_SH_MJ, Manufacture_WH_MJ, Manufacture_MJ,
  ...
  mutate(Manufacture_SH = Manufacture_SH*21,
         ...
         EOL_S= EOL_S*21,
         Other_S = Other_S*21,
         Manufacture_SH_MJ = Manufacture_SH_MJ*21,
         Distribution_SH_MJ = Distribution_SH_MJ*21,
         Installation_SH_MJ = Installation_SH_MJ*21,
         EOL_SH_MJ= EOL_SH_MJ*21,
         Other_SH_MJ = Other_SH_MJ*21,
         Manufacture_WH_MJ = Manufacture_WH_MJ*21,
         Distribution_WH_MJ = Distribution_WH_MJ*21,
         Installation_WH_MJ = Installation_WH_MJ*21,
         EOL_WH_MJ= EOL_WH_MJ*21,
         Other_WH_MJ = Other_WH_MJ*21,
         Manufacture_MJ = Manufacture_MJ*21,
         Distribution_MJ = Distribution_MJ*21,
         Installation_MJ = Installation_MJ*21,
         EOL_MJ= EOL_MJ*21,
...
household_lifetime <- household_operation %>%
  ...
  mutate(Embodied_life = Manufacture + Distribution + Installation + EOL + Other,
         ...
         Embodied_life_MJ = Manufacture_MJ + Distribution_MJ + Installation_MJ + EOL_MJ + Other_MJ,
         Total_life_MJ =  (P_annual_kWh_life/0.278) + Embodied_life_MJ)

# Please make sure you change to desired directory (latest version was saved in github repo)
household_lifetime %>% write_csv(paste0("D:/EECA_SWH_LCA/EECA_SWH_LCA/EECA_SWH_LCA/data/processed/lca/household_lifetime_env_shifted_ocgt_",CS,".csv"))
