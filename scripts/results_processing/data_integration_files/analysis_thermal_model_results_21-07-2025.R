...
# Define currenyt control signal
...
technology_year <- raw_technologies_processed_CS_DW_SH_HYD %>% 
...
technology_year %>% write_csv(paste0("D:/EECA_SWH_LCA/EECA_SWH_LCA/EECA_SWH_LCA/data/processed/lca/technology_year_",CS,"_21_07_2025.csv"))
...
# Please make sure you change to desired directory (latest version was saved in github repo)
technology_lifetime %>% write_csv(paste0("D:/EECA_SWH_LCA/EECA_SWH_LCA/EECA_SWH_LCA/data/processed/lca/technology_lifetime_",CS,"_21_07_2025.csv"))
...
# Please make sure you change to desired directory (latest version was saved in github repo)
household %>% write_csv(paste0("D:/EECA_SWH_LCA/EECA_SWH_LCA/EECA_SWH_LCA/data/processed/lca/households_",CS,"_21_07_2025.csv"))
...
household_lifetime <- household_interp_lifetime_operational %>%
  left_join(household_embodied, 
            by = c("house_type","insulation","schedule",
                   "Loc_code","Hydro_resource",
                   "SH_DW_code","Occ_code")) %>%
  mutate(kgCO2_total_life = kgCO2_op_life + Manufacture + Distribution + Installation + EOL + Other)

# Please make sure you change to desired directory (latest version was saved in github repo)
household_lifetime %>% write_csv(paste0("D:/EECA_SWH_LCA/EECA_SWH_LCA/EECA_SWH_LCA/data/processed/lca/households_lifetime_",CS,"_21_07_2025.csv"))
...
stacked_household_lifetime <- household_lifetime %>% 
  ...
  summarise(
    ...
    Manufacture    = mean(Manufacture,        na.rm = TRUE),
    Distribution    = mean(Distribution,        na.rm = TRUE),
    Installation = mean(Installation,        na.rm = TRUE),
    EOL = mean(EOL,        na.rm = TRUE),
    Other = mean(Other,        na.rm = TRUE),
    .groups = "drop"
  ) %>% 
  pivot_longer(
    cols      = c(Operational, Manufacture, Distribution,
                  Installation, EOL, Other),
    names_to  = "component",
    values_to = "value"
  ) %>% 
  group_by(SH_DW_code, x_num, insulation, Loc_code) %>% 
...
ggplot(household_lifetime, aes(x = as.factor(x_num), y = kgCO2_total_life, group = interaction(SH_DW_code, insulation), fill = insulation)) +
  ...
  labs(
    title = "Distribution of Lifecycle Emissions (Manufacture + Operational) for SWH combinations",
    x = 'SWH setup',
    y = "kgCO2e",
    fill = "Insulation"
  ) +
...
y_max <- max(stacked_household_lifetime_plot$value) * 1.5
